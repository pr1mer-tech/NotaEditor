...
class OpenQuicklyViewController: NSViewController, NSTextFieldDelegate {
    ...
    func keyDown(with event: NSEvent) -> NSEvent? {
        ...
        if keyCode == KeyCode.esc {
            openQuicklyWindowController?.close()
            return nil
        }
    ...
    @objc func itemSelected() {
        ...
        openQuicklyWindowController?.toggle(with: .zero)
    }
    ...
    private func updateViewSize() {
        ...
        let newHeight = options.height + rowHeight + (matches.count > 0 ? options.edgeInsets.top + options.edgeInsets.bottom : 0)
