...
public struct Theme {
    ...
    public init(_ name: String) {
        let bundle = Bundle(for: EditorCore.self)
        
        let path: String
        
        if let path1 = bundle.path(forResource: "EditorCore.framework/themes/\(name)", ofType: "json") {
            
            path = path1
        }
        else if let path2 = bundle.path(forResource: "EditorCore.framework/\(name)", ofType: "json") {
            ...
            print("[EditorCore] Unable to load your theme file.")
            assertionFailure()
            return
        }
    ...
    /// Sets the background color, tint color, etc. of the EditorCore editor.
    ///
